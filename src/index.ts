...
import { downloadVideo, getInfo, sendVideo } from './download-video';
  ...
  await Promise.all(
    entities
      ...
      .map(async (url) => {
        ...
        const log = new LogMessage(ctx, `⬇️ <b>Downloading</b> ${url}...`);
        try {
          const info = await getInfo(log, url, verbose);
          url = info.webpage_url || url;
          console.log(await downloadVideo(log, url, verbose));
          await sendVideo(ctx, log, info, ctx.chat.id, message_id, verbose);
        } catch (e: any) {
