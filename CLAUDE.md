...
- **NO SILENT FAILURES.** If something fails, it must fail loudly — throw an error, log a visible message, make it obvious. Never swallow errors, return empty defaults, or silently skip steps. No exceptions. If a required credential or dependency is missing, CRASH. Do not wrap it in a try/catch that returns undefined. A test that skips when it fails is not a test.
