...
- **NO SILENT FAILURES.** If something fails, it must fail loudly — throw an error, log a visible message, make it obvious. Never swallow errors, return empty defaults, or silently skip steps. The only exception is when the failure is expected/unavoidable (e.g., yt-dlp services requiring auth cookies in CI) — in that case, log a visible skip message. If a required credential or dependency is missing, CRASH. Do not wrap it in a try/catch that returns undefined.
